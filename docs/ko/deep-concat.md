...
<pre class="lrbox">
   <span class="nt">A</span> → α <span class="lrdot">•</span> X β       (α = 이미 읽음,  X = 지금 볼 기호,  β = 남음)
</pre>
...
<pre class="lrbox">
   <span class="nt">A</span> → <span class="nt">Expr</span> <span class="setm">'+'</span> <span class="nt">Term</span>          (인덱스   0     1     2)

   X = <span class="setm">'+'</span> (인덱스 1) 로 보면   →   <span class="nt">A</span> → <span class="nt">Expr</span> <span class="lrdot">•</span> <span class="setm">'+'</span> <span class="nt">Term</span>

   PrevSymbolListFrom(1) = [ <span class="nt">Expr</span> ]        ← α : 점 앞 '전부'  (Take(1))
   PostSymbolListFrom(1) = [ <span class="nt">Term</span> ]        ← β : X 뒤 '전부'   (Skip(2))
   ( 인덱스 1의 <span class="setm">'+'</span> 자신은 어느 쪽에도 안 들어가요 — 지금 '보는' 기호니까 )
</pre>
