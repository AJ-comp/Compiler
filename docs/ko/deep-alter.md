...
<pre class="lrbox">
   ...
   NonTerminalAlter   (= alters,  Concat들의 집합)
     ├ NonTerminalConcat  [ <span class="nt">Expr</span> ] · [ <span class="setm">'+'</span> ] · [ <span class="nt">Term</span> ]     ← 대안 1
     └ NonTerminalConcat  [ <span class="nt">Term</span> ]                           ← 대안 2
</pre>
...
<pre class="lrbox">
   AddAsConcat(a, b)   →   Alter { [ <span class="setm">a</span> · <span class="setm">b</span> ] }        ← 대안 1개 (안에 a, b 가 순서대로)

   AddAsAlter(a, b)    →   Alter { [ <span class="setm">a</span> ] , [ <span class="setm">b</span> ] }    ← 대안 2개 (a 하나, b 하나)
</pre>
...
<pre class="lrbox">
   Symbol  (추상)
    ...
    └ NonTerminal "<span class="nt">Expr</span>"    ← 가지(규칙)
         └ alters : NonTerminalAlter            ← 대안들의 '집합'   (Alter)
              ├ NonTerminalConcat [<span class="nt">Expr</span> · <span class="setm">'+'</span> · <span class="nt">Term</span>]   ┐  각 원소(Concat)를
              └ NonTerminalConcat [<span class="nt">Term</span>]                ┘  "Expr의 N번째" 로 보면
...
👉 **[FIRST · FOLLOW — 공식과 구현](first-formula.md)**
