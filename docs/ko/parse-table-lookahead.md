...
| `α` `β` | *기호 묶음* — 단말·비단말이 **0개 이상** 늘어선 토막 | 빈 토막 · `c` · `A c` · `a A` … |
...
점 뒤에 비단말 `B` 가 있죠 (그 뒤엔 토막 `β`, 그리고 이 아이템의 lookahead `t`). 클로저는 `B → γ` 규칙들을 새로 넣어요:
...
원래 아이템에서 `B` 뒤에 있던 토막 `β` 예요. 그러니 `B` 의 lookahead 는 *`β` 의 첫 글자* 들 —

> 새 lookahead = `FIRST(β)`

그런데 만약 **`β` 가 비었거나 통째로 사라질 수 있으면** (즉 `B` 뒤에 사실상 아무것도 없으면), 그땐 `B` 다음에 오는 건 *원래 아이템의 lookahead `t`* 예요. 이 "그럴 땐 `t` 를 물려받기" 까지 한 번에 적은 게 —

> ### 새 lookahead = `FIRST( β t )`

(`FIRST(β t)` = `β` 의 FIRST 를 모으되, `β` 가 전부 사라질 수 있으면 `t` 까지 포함 — [FIRST](first-formula.md) 정의 그대로예요.)

이게 lookahead 를 구하는 **공식의 전부** 예요. *새로운 계산이 아니라*, 이미 배운 **FIRST 를 "점 뒤 토막 `β`" 에 적용** 하는 것뿐이에요.
...
- `A` 다음에 남은 토막 `β` = `)`
- 이 아이템의 lookahead `t` = `$`
- → 새 lookahead = `FIRST( ) $ )` = `FIRST( ) )` = <code><span class="setb">{</span><span class="setm"> ) </span><span class="setb">}</span></code> &nbsp; (`)` 는 단말이라 자기 자신)
...
**`β` 가 빈 경우도 한 번 봐요.** 만약 규칙이 `S → ( A )` 가 아니라 그냥 `S → A` 였다면? `[ S → • A , $ ]` 에서 `A` 를 펼칠 때 — `A` 뒤에 남은 토막이 *없어요* (`β` 가 빔). 그럼 공식이 `FIRST( $ )` = <code><span class="setb">{</span><span class="setm"> $ </span><span class="setb">}</span></code>, 즉 **부모의 lookahead `$` 를 그대로 물려받아** `[ A → • n , $ ]` 가 돼요.

> 🔖 **한 줄 정리** — lookahead 는 *아이템마다 붙는 "접어도 되는 다음 단말"* 이고, 클로저에서 **`FIRST(β t)`** 로 구해요. (점 뒤 토막 `β` 의 FIRST, `β` 가 사라지면 부모 `t` 를 물려받기.)
