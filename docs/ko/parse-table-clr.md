...
[SLR 장](parse-table-slr.md) 에서 SLR 의 약점 — **헛충돌** — 을 봤어요. 뿌리는, SLR 이 reduce 를 정할 때 `FOLLOW` 를 *문법 전체* 에서 끌어온 데 있었죠. 그래서 *지금 이 상태* 와 무관한 글자까지 함께 포함됐고요.
...
이 둘을 *모두 합친* 게 <code>FOLLOW(A) = <span class="setb">{</span><span class="setm"> c d </span><span class="setb">}</span></code> 예요 — SLR 이 쓰는 *포괄적인* 집합이 바로 이거죠.
