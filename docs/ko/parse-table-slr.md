...
> 이건 [충돌이란?](parse-table-conflict.md) 장의 *진짜* reduce/reduce 와는 달라요. 거긴 문법이 *정말로* 모호했지만(같은 `a` 를 A 로도 B 로도 볼 수 있었죠), 여긴 모호하지 않은데 SLR 이 *헛것* 을 본 거예요. — 바로 **사다리 윗 칸(CLR)이 정확히 고치는 지점** 이고요. (CLR 장에서 *바로 이 문법* 을 다시 펴 보여드릴게요.)
...
이 헛충돌은 — FOLLOW *전체* 말고 **"바로 이 상태에 진짜로 도달하는 글자"** 에만 reduce 하면 사라져요. 그 *"이 자리에 딱 맞는 글자"* 가 바로 다음 장의 **lookahead** 고 — 그걸 상태에 처음부터 박는 게 **CLR**, 합쳐서 줄인 게 **LALR** 이에요. 차근차근 올라갑니다.

그 전에 — 방금 배운 SLR 이 *엔진 코드* 로는 어떻게 생겼는지, 딱 한 페이지만 보고 가요. (말로 한 정의가 코드에 그대로 박혀 있어서, 한 번 보면 더 또렷해져요.)
