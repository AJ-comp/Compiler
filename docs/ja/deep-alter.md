...
> 🎓 **発展課程** です。ついに [NonTerminal](deep-nonterminal.md) が抱えていたあの **`alters` 箱**
...
[FIRST/FOLLOW](first-follow.md) を計算するとき「この規則は空文字列を作れるか?」が重要でしたよね。\
その判断をここでそのまま答えてくれます。(断片たちがどうつながるのか、だんだん見えてきましたよね?)
...
求める段階です。\
基本課程で概念として出会ったそれを、今度は **公式とコード** で。

👉 **FIRST / FOLLOW — 公式と実装** *(作成予定)*
