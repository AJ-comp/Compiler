...
> 🎓 **発展コース · 実装** です。\
> 前の [FOLLOW · 計算規則](follow-rules.md)で、三つの規則（① 開始記号 `$` / ② 後ろの `FIRST` / ③ 一番後ろなら LHS を継承）と反復を見ましたね。\
...
生成規則を **一番後ろ（右）から** 見ながら、後ろにある非終端記号に `FOLLOW(A)` を注ぎ込みます → **規則 ③**。\
...
最初の行 `CalculateAllFirst` が、*「FOLLOW は FIRST を材料に使う」* をコードで言い切っているんですね。
...
| 規則 ③ — 一番後ろなら LHS 継承 | `ConCatExprUpdateFollow`（右から `UnionWith(followSet)`） |
