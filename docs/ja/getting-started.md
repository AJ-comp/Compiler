...
構文解析はほぼすべての言語処理の
**最初の関門**です。\
さあ — この「構文解析」を、これから Orchid で実際にやってみましょう。
...
> パッケージ名がなぜ **`Orchidaceae`** なのかというと — ブランド名「Orchid」が nuget.org にすでに予約されているので、
> ラン科(科)の学名である `Orchidaceae` を id として使っています。`--prerelease` はまだ初期プレビューなので付けます。
