...
> 📍 **在りか** · `Analyzer.Goto` · `…/Parsers/Analyzer.cs`
...
<pre class="lrbox">   Factor → '(' <span class="lrdot">•</span> Expr ')'        <span style="opacity:.65">← ドットを動かしたもの</span>
   Expr   → <span class="lrdot">•</span> Expr '+' Term         <span style="opacity:.65">← 閉包で満たされた</span>
   Expr   → <span class="lrdot">•</span> Term
   Term   → <span class="lrdot">•</span> Term '*' Factor
   Term   → <span class="lrdot">•</span> Factor
   Factor → <span class="lrdot">•</span> '(' Expr ')'
