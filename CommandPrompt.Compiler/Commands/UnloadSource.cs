...
{
    public class UnloadSource : Command
    {
        public UnloadSource(string name, string description = null) : base(name, description)
        {
        ...
        private int Process(string srcPathWithFile, string projFullPath)
        {
            return this.Execute(() =>
            {
                ...
                if (pathInfo.Relative) project.RemoveFiles.Add(new PathModel(pathInfo.TargetRelativeFullPath));
                else project.ExternalFiles.Remove(new PathModel(pathInfo.TargetAbsoluteFullPath));
